...
------------- | ------------- | -------------
[**AppsGet**](AppsAPI.md#AppsGet) | **Get** /api/2.45/apps | List apps
[**AppsNodesGet**](AppsAPI.md#AppsNodesGet) | **Get** /api/2.45/apps/nodes | List app nodes
[**AppsPatch**](AppsAPI.md#AppsPatch) | **Patch** /api/2.45/apps | Modify app



## AppsGet

> AppGetResponse AppsGet(ctx).Authorization(authorization).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.AppsAPI.AppsGet(context.Background())
    ...
    // response from `AppsGet`: AppGetResponse
...
[**AppGetResponse**](AppGetResponse.md)
...
> AppNodeGetResponse AppsNodesGet(ctx).Authorization(authorization).XRequestID(xRequestID).AppNames(appNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.AppsAPI.AppsNodesGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `AppsAPI.AppsNodesGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `AppsNodesGet`: AppNodeGetResponse
    fmt.Fprintf(os.Stdout, "Response from `AppsAPI.AppsNodesGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiAppsNodesGetRequest struct via the builder pattern
 ...
 **authorization** | **string** | Access token (in JWT format) required to use any API endpoint (except &#x60;/oauth2&#x60;, &#x60;/login&#x60;, and &#x60;/logout&#x60;)  | 
 **xRequestID** | **string** | Supplied by client during request or generated by server.  | 
 **appNames** | **[]string** | The name of the installed app. Enter multiple names in comma-separated format.  | 
 **continuationToken** | **string** | A token used to retrieve the next page of data with some consistency guaranteed. The token is a Base64 encoded value. Set &#x60;continuation_token&#x60; to the system-generated token taken from the &#x60;x-next-token&#x60; header field of the response. A query has reached its last page when the response does not include a token. Pagination requires the &#x60;limit&#x60; and &#x60;continuation_token&#x60; query parameters.  | 
 **filter** | **string** | Narrows down the results to only the response objects that satisfy the filter criteria.  | 
 **limit** | **int32** | Limits the size of the response to the specified number of objects on each page. To return the total number of resources, set &#x60;limit&#x3D;0&#x60;. The total number of resources is returned as a &#x60;total_item_count&#x60; value. If the page size requested is larger than the system maximum limit, the server returns the maximum limit, disregarding the requested page size.  | 
 **offset** | **int32** | The starting position based on the results of the query in relation to the full set of response objects returned.  | 
...
[**AppNodeGetResponse**](AppNodeGetResponse.md)
...
> AppResponse AppsPatch(ctx).Names(names).App(app).Authorization(authorization).XRequestID(xRequestID).Execute()
...
func main() {
    ...
    req := apiClient.AppsAPI.AppsPatch(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `AppsAPI.AppsPatch``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `AppsPatch`: AppResponse
    fmt.Fprintf(os.Stdout, "Response from `AppsAPI.AppsPatch`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiAppsPatchRequest struct via the builder pattern
