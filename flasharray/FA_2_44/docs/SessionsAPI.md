...
------------- | ------------- | -------------
[**SessionsGet**](SessionsAPI.md#SessionsGet) | **Get** /api/2.44/sessions | List session data



## SessionsGet

> SessionGetResponse SessionsGet(ctx).Authorization(authorization).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
