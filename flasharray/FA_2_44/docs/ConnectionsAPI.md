...
------------- | ------------- | -------------
[**ConnectionsDelete**](ConnectionsAPI.md#ConnectionsDelete) | **Delete** /api/2.44/connections | Delete a connection between a volume and its host or host group
[**ConnectionsGet**](ConnectionsAPI.md#ConnectionsGet) | **Get** /api/2.44/connections | List volume connections
[**ConnectionsPost**](ConnectionsAPI.md#ConnectionsPost) | **Post** /api/2.44/connections | Create a connection between a volume and host or host group



## ConnectionsDelete

> ConnectionsDelete(ctx).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).HostGroupNames(hostGroupNames).HostNames(hostNames).VolumeIds(volumeIds).VolumeNames(volumeNames).Execute()
...
> ConnectionGetResponse ConnectionsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).HostGroupNames(hostGroupNames).HostNames(hostNames).Limit(limit).Offset(offset).ProtocolEndpointIds(protocolEndpointIds).ProtocolEndpointNames(protocolEndpointNames).Sort(sort).TotalItemCount(totalItemCount).VolumeIds(volumeIds).VolumeNames(volumeNames).Execute()
...
> ConnectionResponse ConnectionsPost(ctx).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).HostGroupNames(hostGroupNames).HostNames(hostNames).VolumeIds(volumeIds).VolumeNames(volumeNames).Connection(connection).Execute()
