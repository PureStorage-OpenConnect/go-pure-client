...
------------- | ------------- | -------------
[**ConnectionsDelete**](ConnectionsAPI.md#ConnectionsDelete) | **Delete** /api/2.2/connections | Delete a connection between a volume and its host or host group
[**ConnectionsGet**](ConnectionsAPI.md#ConnectionsGet) | **Get** /api/2.2/connections | List volume connections
[**ConnectionsPost**](ConnectionsAPI.md#ConnectionsPost) | **Post** /api/2.2/connections | Create a connection between a volume and host or host group



## ConnectionsDelete

> ConnectionsDelete(ctx).Authorization(authorization).XRequestID(xRequestID).HostGroupNames(hostGroupNames).HostNames(hostNames).VolumeNames(volumeNames).Execute()
...
> ConnectionGetResponse ConnectionsGet(ctx).Authorization(authorization).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).HostGroupNames(hostGroupNames).HostNames(hostNames).Limit(limit).Offset(offset).ProtocolEndpointNames(protocolEndpointNames).Sort(sort).TotalItemCount(totalItemCount).VolumeNames(volumeNames).Execute()
...
> ConnectionResponse ConnectionsPost(ctx).Authorization(authorization).XRequestID(xRequestID).HostGroupNames(hostGroupNames).HostNames(hostNames).VolumeNames(volumeNames).Connection(connection).Execute()
