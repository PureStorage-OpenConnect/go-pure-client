/*
FlashArray REST API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.53
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gopureclient

import (
	"encoding/json"
)

// checks if the ResourceRealmSpace type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ResourceRealmSpace{}

// ResourceRealmSpace struct for ResourceRealmSpace
type ResourceRealmSpace struct {
	// A globally unique, system-generated ID. The ID cannot be modified and cannot refer to another resource.
	Id *string `json:"id,omitempty"`
	// A locally unique, system-generated name. The name cannot be modified.
	Name *string `json:"name,omitempty"`
	// Displays size and space consumption information.
	Space *SpaceNoDeprecatedPhysicalOrEffective `json:"space,omitempty"`
	// The timestamp of when the data was taken, measured in milliseconds since the UNIX epoch.
	Time *int64 `json:"time,omitempty"`
}

// NewResourceRealmSpace instantiates a new ResourceRealmSpace object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewResourceRealmSpace() *ResourceRealmSpace {
	this := ResourceRealmSpace{}
	return &this
}

// NewResourceRealmSpaceWithDefaults instantiates a new ResourceRealmSpace object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewResourceRealmSpaceWithDefaults() *ResourceRealmSpace {
	this := ResourceRealmSpace{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *ResourceRealmSpace) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResourceRealmSpace) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *ResourceRealmSpace) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *ResourceRealmSpace) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *ResourceRealmSpace) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResourceRealmSpace) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *ResourceRealmSpace) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *ResourceRealmSpace) SetName(v string) {
	o.Name = &v
}

// GetSpace returns the Space field value if set, zero value otherwise.
func (o *ResourceRealmSpace) GetSpace() SpaceNoDeprecatedPhysicalOrEffective {
	if o == nil || IsNil(o.Space) {
		var ret SpaceNoDeprecatedPhysicalOrEffective
		return ret
	}
	return *o.Space
}

// GetSpaceOk returns a tuple with the Space field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResourceRealmSpace) GetSpaceOk() (*SpaceNoDeprecatedPhysicalOrEffective, bool) {
	if o == nil || IsNil(o.Space) {
		return nil, false
	}
	return o.Space, true
}

// HasSpace returns a boolean if a field has been set.
func (o *ResourceRealmSpace) HasSpace() bool {
	if o != nil && !IsNil(o.Space) {
		return true
	}

	return false
}

// SetSpace gets a reference to the given SpaceNoDeprecatedPhysicalOrEffective and assigns it to the Space field.
func (o *ResourceRealmSpace) SetSpace(v SpaceNoDeprecatedPhysicalOrEffective) {
	o.Space = &v
}

// GetTime returns the Time field value if set, zero value otherwise.
func (o *ResourceRealmSpace) GetTime() int64 {
	if o == nil || IsNil(o.Time) {
		var ret int64
		return ret
	}
	return *o.Time
}

// GetTimeOk returns a tuple with the Time field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ResourceRealmSpace) GetTimeOk() (*int64, bool) {
	if o == nil || IsNil(o.Time) {
		return nil, false
	}
	return o.Time, true
}

// HasTime returns a boolean if a field has been set.
func (o *ResourceRealmSpace) HasTime() bool {
	if o != nil && !IsNil(o.Time) {
		return true
	}

	return false
}

// SetTime gets a reference to the given int64 and assigns it to the Time field.
func (o *ResourceRealmSpace) SetTime(v int64) {
	o.Time = &v
}

func (o ResourceRealmSpace) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ResourceRealmSpace) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Space) {
		toSerialize["space"] = o.Space
	}
	if !IsNil(o.Time) {
		toSerialize["time"] = o.Time
	}
	return toSerialize, nil
}

type NullableResourceRealmSpace struct {
	value *ResourceRealmSpace
	isSet bool
}

func (v NullableResourceRealmSpace) Get() *ResourceRealmSpace {
	return v.value
}

func (v *NullableResourceRealmSpace) Set(val *ResourceRealmSpace) {
	v.value = val
	v.isSet = true
}

func (v NullableResourceRealmSpace) IsSet() bool {
	return v.isSet
}

func (v *NullableResourceRealmSpace) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableResourceRealmSpace(val *ResourceRealmSpace) *NullableResourceRealmSpace {
	return &NullableResourceRealmSpace{value: val, isSet: true}
}

func (v NullableResourceRealmSpace) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableResourceRealmSpace) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
