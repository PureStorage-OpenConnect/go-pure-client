...
------------- | ------------- | -------------
[**ControllersGet**](ControllersAPI.md#ControllersGet) | **Get** /api/2.53/controllers | List controller information and status



## ControllersGet

> ControllerGetResponse ControllersGet(ctx).Authorization(authorization).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.ControllersAPI.ControllersGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `ControllersAPI.ControllersGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ControllersGet`: ControllerGetResponse
    fmt.Fprintf(os.Stdout, "Response from `ControllersAPI.ControllersGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiControllersGetRequest struct via the builder pattern
...
[**ControllerGetResponse**](ControllerGetResponse.md)
