...
------------- | ------------- | -------------
[**FileSystemsDelete**](FileSystemsAPI.md#FileSystemsDelete) | **Delete** /api/2.51/file-systems | Delete file system
[**FileSystemsGet**](FileSystemsAPI.md#FileSystemsGet) | **Get** /api/2.51/file-systems | List file systems
[**FileSystemsPatch**](FileSystemsAPI.md#FileSystemsPatch) | **Patch** /api/2.51/file-systems | Modify a file system
[**FileSystemsPost**](FileSystemsAPI.md#FileSystemsPost) | **Post** /api/2.51/file-systems | Create file system



## FileSystemsDelete

> FileSystemsDelete(ctx).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Ids(ids).Names(names).Execute()
...
> FileSystemGetResponse FileSystemsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Destroyed(destroyed).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).WorkloadIds(workloadIds).WorkloadNames(workloadNames).Execute()
...
> FileSystemResponse FileSystemsPatch(ctx).FileSystem(fileSystem).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Ids(ids).Names(names).Execute()
...
> FileSystemResponse FileSystemsPost(ctx).Authorization(authorization).XRequestID(xRequestID).AddToPolicyIds(addToPolicyIds).AddToPolicyNames(addToPolicyNames).ContextNames(contextNames).Names(names).WithDefaultProtection(withDefaultProtection).FileSystem(fileSystem).Execute()
 ...
 **authorization** | **string** | Access token (in JWT format) required to use any API endpoint (except &#x60;/oauth2&#x60;, &#x60;/login&#x60;, and &#x60;/logout&#x60;)  | 
 **xRequestID** | **string** | Supplied by client during request or generated by server.  | 
 **addToPolicyIds** | **[]string** | The root directories of the file systems will become members of the specified policies along with creation. Enter multiple ids in comma-separated format.  | 
 **addToPolicyNames** | **[]string** | The root directories of the file systems will become members of the specified policies along with creation. Enter multiple names in a comma-separated format.  | 
 **contextNames** | **[]string** | Performs the operation on the context specified.  If specified, the context names must be an array of size 1, and the single element must be the name of an array in the same fleet or the name of the fleet itself. If not specified, the context will default to the array that received this request.  Other parameters provided with the request, such as names of volumes or snapshots, are resolved relative to the provided &#x60;context&#x60;.  | 
 **names** | **[]string** | Performs the operation on the unique names specified. Enter multiple names in comma-separated format. For example, &#x60;name01,name02&#x60;.  If there is not at least one resource that matches each of the elements of &#x60;names&#x60;, then an error is returned, except when creating new resources.  | 
 **withDefaultProtection** | **bool** | If specified as &#x60;true&#x60;, the initial protection of the root directories of the newly created file systems will be the union of the container default protection configuration and either &#x60;add_to_policy_names&#x60; or &#x60;add_to_policy_ids&#x60;. If specified as &#x60;false&#x60;, the default protection of the container will not be applied automatically. The initial protection of the root directories of the newly created file systems will be configured by either &#x60;add_to_policy_names&#x60; or &#x60;add_to_policy_ids&#x60;. If not specified, defaults to &#x60;true&#x60;.  | 
