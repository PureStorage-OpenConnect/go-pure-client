...
------------- | ------------- | -------------
[**RemoteRealmsGet**](RemoteRealmsAPI.md#RemoteRealmsGet) | **Get** /api/2.51/remote-realms | List remote realms
[**RemoteRealmsTagsGet**](RemoteRealmsAPI.md#RemoteRealmsTagsGet) | **Get** /api/2.51/remote-realms/tags | List tags



## RemoteRealmsGet

> RemoteRealmsResponse RemoteRealmsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).On(on).OnIds(onIds).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.RemoteRealmsAPI.RemoteRealmsGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `RemoteRealmsAPI.RemoteRealmsGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `RemoteRealmsGet`: RemoteRealmsResponse
    fmt.Fprintf(os.Stdout, "Response from `RemoteRealmsAPI.RemoteRealmsGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRemoteRealmsGetRequest struct via the builder pattern
 ...
 **onIds** | **[]string** | Performs the operation on the specified target IDs. Enter multiple target IDs in comma-separated format. The on and on_ids parameters cannot be set together.  | 
...
[**RemoteRealmsResponse**](RemoteRealmsResponse.md)
...
> RemoteRealmTagGetResponse RemoteRealmsTagsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Namespaces(namespaces).Offset(offset).On(on).OnIds(onIds).ResourceIds(resourceIds).ResourceNames(resourceNames).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.RemoteRealmsAPI.RemoteRealmsTagsGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `RemoteRealmsAPI.RemoteRealmsTagsGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `RemoteRealmsTagsGet`: RemoteRealmTagGetResponse
    fmt.Fprintf(os.Stdout, "Response from `RemoteRealmsAPI.RemoteRealmsTagsGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRemoteRealmsTagsGetRequest struct via the builder pattern
...
[**RemoteRealmTagGetResponse**](RemoteRealmTagGetResponse.md)
