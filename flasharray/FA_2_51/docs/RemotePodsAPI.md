...
------------- | ------------- | -------------
[**RemotePodsGet**](RemotePodsAPI.md#RemotePodsGet) | **Get** /api/2.51/remote-pods | List remote pods
[**RemotePodsTagsGet**](RemotePodsAPI.md#RemotePodsTagsGet) | **Get** /api/2.51/remote-pods/tags | List tags



## RemotePodsGet

> RemotePodsResponse RemotePodsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).On(on).OnIds(onIds).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.RemotePodsAPI.RemotePodsGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `RemotePodsAPI.RemotePodsGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `RemotePodsGet`: RemotePodsResponse
    fmt.Fprintf(os.Stdout, "Response from `RemotePodsAPI.RemotePodsGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRemotePodsGetRequest struct via the builder pattern
...
[**RemotePodsResponse**](RemotePodsResponse.md)
...
> RemotePodTagGetResponse RemotePodsTagsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Namespaces(namespaces).Offset(offset).On(on).OnIds(onIds).ResourceIds(resourceIds).ResourceNames(resourceNames).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.RemotePodsAPI.RemotePodsTagsGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `RemotePodsAPI.RemotePodsTagsGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `RemotePodsTagsGet`: RemotePodTagGetResponse
    fmt.Fprintf(os.Stdout, "Response from `RemotePodsAPI.RemotePodsTagsGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRemotePodsTagsGetRequest struct via the builder pattern
...
[**RemotePodTagGetResponse**](RemotePodTagGetResponse.md)
