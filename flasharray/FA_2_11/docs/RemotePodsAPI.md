...
------------- | ------------- | -------------
[**RemotePodsGet**](RemotePodsAPI.md#RemotePodsGet) | **Get** /api/2.11/remote-pods | List remote pods



## RemotePodsGet

> RemotePodsResponse RemotePodsGet(ctx).Authorization(authorization).XRequestID(xRequestID).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).On(on).Sort(sort).TotalItemCount(totalItemCount).Execute()
