...
------------- | ------------- | -------------
[**PortsGet**](PortsAPI.md#PortsGet) | **Get** /api/2.47/ports | List ports
[**PortsInitiatorsGet**](PortsAPI.md#PortsInitiatorsGet) | **Get** /api/2.47/ports/initiators | List port initiators



## PortsGet

> PortGetResponse PortsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
> PortInitiatorsGetResponse PortsInitiatorsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
