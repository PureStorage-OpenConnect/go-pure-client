...
type ArrayConnection struct {
	...
	// Status of the connection. Valid values are `connected`, `connecting`, `partially_connected`, `unbalanced`, and `inactive`. A status of `connected` indicates that arrays are communicating. A status of `connecting` indicates that the array is trying to establish a connection. A status of `partially_connected` indicates that some replication addresses are communicating but others are not. A status of `unbalanced` indicates that the arrays are communicating, but the set of paths is either not redundant or not symmetric. A status of `inactive` indicates that the array does not have an established connection with the other array, but will attempt to connect when required.
	Status   *string   `json:"status,omitempty"`
	Throttle *Throttle `json:"throttle,omitempty"`
	// The type of replication. Valid values are `async-replication`, `sync-replication` and `fleet-mgmt`.
