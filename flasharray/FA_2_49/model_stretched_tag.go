/*
FlashArray REST API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.49
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gopureclient

import (
	"encoding/json"
)

// checks if the StretchedTag type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &StretchedTag{}

// StretchedTag struct for StretchedTag
type StretchedTag struct {
	// The context in which the operation was performed. Valid values include a reference to any array which is a member of the same fleet. If the array is not a member of a fleet, `context` will always implicitly be set to the array that received the request.  Other parameters provided with the request, such as names of volumes or snapshots, are resolved relative to the provided `context`.
	Context *FixedReferenceWithType `json:"context,omitempty"`
	// Specifies whether or not to include the tag when copying the parent resource. If set to `true`, the tag is included in resource copying. If set to `false`, the tag is not included. If not specified, defaults to `true`.
	Copyable *bool `json:"copyable,omitempty"`
	// Key of the tag. Supports up to 64 Unicode characters.
	Key *string `json:"key,omitempty"`
	// Optional namespace of the tag. Namespace identifies the category of the tag. Omitting the namespace defaults to the namespace `default`. The `pure&#42;` namespaces are reserved for plugins and integration partners. It is recommended that customers avoid using reserved namespaces.
	Namespace *string         `json:"namespace,omitempty"`
	Resource  *FixedReference `json:"resource,omitempty"`
	// Value of the tag. Supports up to 256 Unicode characters.
	Value  *string    `json:"value,omitempty"`
	Arrays []Resource `json:"arrays,omitempty"`
}

// NewStretchedTag instantiates a new StretchedTag object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStretchedTag() *StretchedTag {
	this := StretchedTag{}
	return &this
}

// NewStretchedTagWithDefaults instantiates a new StretchedTag object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStretchedTagWithDefaults() *StretchedTag {
	this := StretchedTag{}
	return &this
}

// GetContext returns the Context field value if set, zero value otherwise.
func (o *StretchedTag) GetContext() FixedReferenceWithType {
	if o == nil || IsNil(o.Context) {
		var ret FixedReferenceWithType
		return ret
	}
	return *o.Context
}

// GetContextOk returns a tuple with the Context field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StretchedTag) GetContextOk() (*FixedReferenceWithType, bool) {
	if o == nil || IsNil(o.Context) {
		return nil, false
	}
	return o.Context, true
}

// HasContext returns a boolean if a field has been set.
func (o *StretchedTag) HasContext() bool {
	if o != nil && !IsNil(o.Context) {
		return true
	}

	return false
}

// SetContext gets a reference to the given FixedReferenceWithType and assigns it to the Context field.
func (o *StretchedTag) SetContext(v FixedReferenceWithType) {
	o.Context = &v
}

// GetCopyable returns the Copyable field value if set, zero value otherwise.
func (o *StretchedTag) GetCopyable() bool {
	if o == nil || IsNil(o.Copyable) {
		var ret bool
		return ret
	}
	return *o.Copyable
}

// GetCopyableOk returns a tuple with the Copyable field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StretchedTag) GetCopyableOk() (*bool, bool) {
	if o == nil || IsNil(o.Copyable) {
		return nil, false
	}
	return o.Copyable, true
}

// HasCopyable returns a boolean if a field has been set.
func (o *StretchedTag) HasCopyable() bool {
	if o != nil && !IsNil(o.Copyable) {
		return true
	}

	return false
}

// SetCopyable gets a reference to the given bool and assigns it to the Copyable field.
func (o *StretchedTag) SetCopyable(v bool) {
	o.Copyable = &v
}

// GetKey returns the Key field value if set, zero value otherwise.
func (o *StretchedTag) GetKey() string {
	if o == nil || IsNil(o.Key) {
		var ret string
		return ret
	}
	return *o.Key
}

// GetKeyOk returns a tuple with the Key field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StretchedTag) GetKeyOk() (*string, bool) {
	if o == nil || IsNil(o.Key) {
		return nil, false
	}
	return o.Key, true
}

// HasKey returns a boolean if a field has been set.
func (o *StretchedTag) HasKey() bool {
	if o != nil && !IsNil(o.Key) {
		return true
	}

	return false
}

// SetKey gets a reference to the given string and assigns it to the Key field.
func (o *StretchedTag) SetKey(v string) {
	o.Key = &v
}

// GetNamespace returns the Namespace field value if set, zero value otherwise.
func (o *StretchedTag) GetNamespace() string {
	if o == nil || IsNil(o.Namespace) {
		var ret string
		return ret
	}
	return *o.Namespace
}

// GetNamespaceOk returns a tuple with the Namespace field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StretchedTag) GetNamespaceOk() (*string, bool) {
	if o == nil || IsNil(o.Namespace) {
		return nil, false
	}
	return o.Namespace, true
}

// HasNamespace returns a boolean if a field has been set.
func (o *StretchedTag) HasNamespace() bool {
	if o != nil && !IsNil(o.Namespace) {
		return true
	}

	return false
}

// SetNamespace gets a reference to the given string and assigns it to the Namespace field.
func (o *StretchedTag) SetNamespace(v string) {
	o.Namespace = &v
}

// GetResource returns the Resource field value if set, zero value otherwise.
func (o *StretchedTag) GetResource() FixedReference {
	if o == nil || IsNil(o.Resource) {
		var ret FixedReference
		return ret
	}
	return *o.Resource
}

// GetResourceOk returns a tuple with the Resource field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StretchedTag) GetResourceOk() (*FixedReference, bool) {
	if o == nil || IsNil(o.Resource) {
		return nil, false
	}
	return o.Resource, true
}

// HasResource returns a boolean if a field has been set.
func (o *StretchedTag) HasResource() bool {
	if o != nil && !IsNil(o.Resource) {
		return true
	}

	return false
}

// SetResource gets a reference to the given FixedReference and assigns it to the Resource field.
func (o *StretchedTag) SetResource(v FixedReference) {
	o.Resource = &v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *StretchedTag) GetValue() string {
	if o == nil || IsNil(o.Value) {
		var ret string
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StretchedTag) GetValueOk() (*string, bool) {
	if o == nil || IsNil(o.Value) {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *StretchedTag) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given string and assigns it to the Value field.
func (o *StretchedTag) SetValue(v string) {
	o.Value = &v
}

// GetArrays returns the Arrays field value if set, zero value otherwise.
func (o *StretchedTag) GetArrays() []Resource {
	if o == nil || IsNil(o.Arrays) {
		var ret []Resource
		return ret
	}
	return o.Arrays
}

// GetArraysOk returns a tuple with the Arrays field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *StretchedTag) GetArraysOk() ([]Resource, bool) {
	if o == nil || IsNil(o.Arrays) {
		return nil, false
	}
	return o.Arrays, true
}

// HasArrays returns a boolean if a field has been set.
func (o *StretchedTag) HasArrays() bool {
	if o != nil && !IsNil(o.Arrays) {
		return true
	}

	return false
}

// SetArrays gets a reference to the given []Resource and assigns it to the Arrays field.
func (o *StretchedTag) SetArrays(v []Resource) {
	o.Arrays = v
}

func (o StretchedTag) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o StretchedTag) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Context) {
		toSerialize["context"] = o.Context
	}
	if !IsNil(o.Copyable) {
		toSerialize["copyable"] = o.Copyable
	}
	if !IsNil(o.Key) {
		toSerialize["key"] = o.Key
	}
	if !IsNil(o.Namespace) {
		toSerialize["namespace"] = o.Namespace
	}
	if !IsNil(o.Resource) {
		toSerialize["resource"] = o.Resource
	}
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	if !IsNil(o.Arrays) {
		toSerialize["arrays"] = o.Arrays
	}
	return toSerialize, nil
}

type NullableStretchedTag struct {
	value *StretchedTag
	isSet bool
}

func (v NullableStretchedTag) Get() *StretchedTag {
	return v.value
}

func (v *NullableStretchedTag) Set(val *StretchedTag) {
	v.value = val
	v.isSet = true
}

func (v NullableStretchedTag) IsSet() bool {
	return v.isSet
}

func (v *NullableStretchedTag) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStretchedTag(val *StretchedTag) *NullableStretchedTag {
	return &NullableStretchedTag{value: val, isSet: true}
}

func (v NullableStretchedTag) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStretchedTag) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
