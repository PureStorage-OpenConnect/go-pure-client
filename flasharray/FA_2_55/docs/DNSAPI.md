...
------------- | ------------- | -------------
[**DnsDelete**](DNSAPI.md#DnsDelete) | **Delete** /api/2.55/dns | Delete DNS configuration
[**DnsGet**](DNSAPI.md#DnsGet) | **Get** /api/2.55/dns | List DNS parameters
[**DnsPatch**](DNSAPI.md#DnsPatch) | **Patch** /api/2.55/dns | Modify DNS parameters
[**DnsPost**](DNSAPI.md#DnsPost) | **Post** /api/2.55/dns | Create DNS configuration



## DnsDelete

> DnsDelete(ctx).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Names(names).Execute()
...
> DnsGetResponse DnsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
> DnsResponse DnsPatch(ctx).Dns(dns).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Names(names).Execute()
...
> DnsResponse DnsPost(ctx).Dns(dns).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Names(names).Execute()
