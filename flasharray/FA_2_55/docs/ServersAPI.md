...
------------- | ------------- | -------------
[**ServersDelete**](ServersAPI.md#ServersDelete) | **Delete** /api/2.55/servers | Delete server
[**ServersGet**](ServersAPI.md#ServersGet) | **Get** /api/2.55/servers | List servers
[**ServersPatch**](ServersAPI.md#ServersPatch) | **Patch** /api/2.55/servers | Modify server
[**ServersPoliciesTlsDelete**](ServersAPI.md#ServersPoliciesTlsDelete) | **Delete** /api/2.55/servers/policies/tls | Delete membership between TLS policies and attached object
[**ServersPoliciesTlsGet**](ServersAPI.md#ServersPoliciesTlsGet) | **Get** /api/2.55/servers/policies/tls | List TLS policies attached to servers
[**ServersPoliciesTlsPost**](ServersAPI.md#ServersPoliciesTlsPost) | **Post** /api/2.55/servers/policies/tls | Create a membership between a server and a TLS policy
[**ServersPost**](ServersAPI.md#ServersPost) | **Post** /api/2.55/servers | Create server



## ServersDelete

> ServersDelete(ctx).Authorization(authorization).XRequestID(xRequestID).CascadeDelete(cascadeDelete).ContextNames(contextNames).Ids(ids).Names(names).Execute()
...
> ServerGetResponse ServersGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).TotalOnly(totalOnly).Execute()
...
> ServerResponse ServersPatch(ctx).Server(server).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Ids(ids).Names(names).Execute()
...
## ServersPoliciesTlsDelete

> ServersPoliciesTlsDelete(ctx).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).MemberIds(memberIds).MemberNames(memberNames).PolicyIds(policyIds).PolicyNames(policyNames).Execute()
...
func main() {
    ...
    req := apiClient.ServersAPI.ServersPoliciesTlsDelete(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `ServersAPI.ServersPoliciesTlsDelete``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
}
```
...
Other parameters are passed through a pointer to a apiServersPoliciesTlsDeleteRequest struct via the builder pattern
...
## ServersPoliciesTlsGet

> PolicyMemberCleanGetResponse ServersPoliciesTlsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).MemberIds(memberIds).MemberNames(memberNames).Offset(offset).PolicyIds(policyIds).PolicyNames(policyNames).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.ServersAPI.ServersPoliciesTlsGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `ServersAPI.ServersPoliciesTlsGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ServersPoliciesTlsGet`: PolicyMemberCleanGetResponse
    fmt.Fprintf(os.Stdout, "Response from `ServersAPI.ServersPoliciesTlsGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiServersPoliciesTlsGetRequest struct via the builder pattern
...
## ServersPoliciesTlsPost

> PolicyMemberCleanResponse ServersPoliciesTlsPost(ctx).Policies(policies).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).MemberIds(memberIds).MemberNames(memberNames).Execute()
...
func main() {
    ...
    req := apiClient.ServersAPI.ServersPoliciesTlsPost(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `ServersAPI.ServersPoliciesTlsPost``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ServersPoliciesTlsPost`: PolicyMemberCleanResponse
    fmt.Fprintf(os.Stdout, "Response from `ServersAPI.ServersPoliciesTlsPost`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiServersPoliciesTlsPostRequest struct via the builder pattern
...
> ServerResponse ServersPost(ctx).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).CreateLocalDirectoryService(createLocalDirectoryService).Names(names).Server(server).Execute()
