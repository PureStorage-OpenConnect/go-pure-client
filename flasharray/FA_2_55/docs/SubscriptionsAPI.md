...
------------- | ------------- | -------------
[**SubscriptionsGet**](SubscriptionsAPI.md#SubscriptionsGet) | **Get** /api/2.55/subscriptions | List subscriptions



## SubscriptionsGet

> SubscriptionGetResponse SubscriptionsGet(ctx).Authorization(authorization).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
func main() {
    ...
    req := apiClient.SubscriptionsAPI.SubscriptionsGet(context.Background())
    ...
    // response from `SubscriptionsGet`: SubscriptionGetResponse
