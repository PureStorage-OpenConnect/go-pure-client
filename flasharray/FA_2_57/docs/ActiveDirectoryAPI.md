...
------------- | ------------- | -------------
[**ActiveDirectoryDelete**](ActiveDirectoryAPI.md#ActiveDirectoryDelete) | **Delete** /api/2.57/active-directory | Delete Active Directory account
[**ActiveDirectoryGet**](ActiveDirectoryAPI.md#ActiveDirectoryGet) | **Get** /api/2.57/active-directory | List Active Directory accounts
[**ActiveDirectoryPatch**](ActiveDirectoryAPI.md#ActiveDirectoryPatch) | **Patch** /api/2.57/active-directory | Modify Active Directory account
[**ActiveDirectoryPost**](ActiveDirectoryAPI.md#ActiveDirectoryPost) | **Post** /api/2.57/active-directory | Create Active Directory account
[**ActiveDirectoryTestGet**](ActiveDirectoryAPI.md#ActiveDirectoryTestGet) | **Get** /api/2.57/active-directory/test | GET active-directory/test
[**ActiveDirectoryTestPatch**](ActiveDirectoryAPI.md#ActiveDirectoryTestPatch) | **Patch** /api/2.57/active-directory/test | PATCH active-directory/test
...
> TestResultWithResourcePatchResponse ActiveDirectoryTestPatch(ctx).Names(names).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).ActiveDirectory(activeDirectory).Execute()
...
func main() {
    ...
    req := apiClient.ActiveDirectoryAPI.ActiveDirectoryTestPatch(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `ActiveDirectoryAPI.ActiveDirectoryTestPatch``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `ActiveDirectoryTestPatch`: TestResultWithResourcePatchResponse
    fmt.Fprintf(os.Stdout, "Response from `ActiveDirectoryAPI.ActiveDirectoryTestPatch`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiActiveDirectoryTestPatchRequest struct via the builder pattern
 ...
 **xRequestID** | **string** | Supplied by client during request or generated by server.  | 
 **contextNames** | **[]string** | Performs the operation on the context specified.  If specified, the context names must be an array of size 1, and the single element must be the name of an array in the same fleet or the name of the fleet itself. If not specified, the context will default to the array that received this request.  Other parameters provided with the request, such as names of volumes or snapshots, are resolved relative to the provided &#x60;context&#x60;.  | 
 **activeDirectory** | [**ActiveDirectoryPatch**](ActiveDirectoryPatch.md) | An optional Active Directory configuration that, if provided, will be used to overwrite aspects of the existing Active Directory configuration when performing tests.  | 
