...
------------- | ------------- | -------------
[**ConnectionsDelete**](ConnectionsAPI.md#ConnectionsDelete) | **Delete** /api/2.0/connections | Break a connection between a volume and its host or host group
[**ConnectionsGet**](ConnectionsAPI.md#ConnectionsGet) | **Get** /api/2.0/connections | List volume connections
[**ConnectionsPost**](ConnectionsAPI.md#ConnectionsPost) | **Post** /api/2.0/connections | Create a connection between a volume and host or host group
...
> ConnectionGetResponse ConnectionsGet(ctx).Authorization(authorization).XRequestID(xRequestID).Filter(filter).HostGroupNames(hostGroupNames).HostNames(hostNames).Limit(limit).Offset(offset).ProtocolEndpointNames(protocolEndpointNames).Sort(sort).TotalItemCount(totalItemCount).VolumeNames(volumeNames).Execute()
