...
------------- | ------------- | -------------
[**DnsGet**](DNSAPI.md#DnsGet) | **Get** /api/2.10/dns | List DNS parameters
[**DnsPatch**](DNSAPI.md#DnsPatch) | **Patch** /api/2.10/dns | Modify DNS parameters



## DnsGet

> DnsGetResponse DnsGet(ctx).Authorization(authorization).XRequestID(xRequestID).Filter(filter).Limit(limit).Offset(offset).Sort(sort).TotalItemCount(totalItemCount).Execute()
...
> DnsResponse DnsPatch(ctx).Dns(dns).Authorization(authorization).XRequestID(xRequestID).Execute()
