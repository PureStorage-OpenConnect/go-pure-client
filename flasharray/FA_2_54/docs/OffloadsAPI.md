...
------------- | ------------- | -------------
[**OffloadsDelete**](OffloadsAPI.md#OffloadsDelete) | **Delete** /api/2.54/offloads | Delete offload target
[**OffloadsGet**](OffloadsAPI.md#OffloadsGet) | **Get** /api/2.54/offloads | List offload targets
[**OffloadsPatch**](OffloadsAPI.md#OffloadsPatch) | **Patch** /api/2.54/offloads | Patch offload target
[**OffloadsPost**](OffloadsAPI.md#OffloadsPost) | **Post** /api/2.54/offloads | Create offload target



## OffloadsDelete

> OffloadsDelete(ctx).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Names(names).Execute()
...
> OffloadGetResponse OffloadsGet(ctx).Authorization(authorization).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Limit(limit).Names(names).Offset(offset).Protocol(protocol).Sort(sort).TotalItemCount(totalItemCount).TotalOnly(totalOnly).Execute()
...
> OffloadResponse OffloadsPatch(ctx).Offload(offload).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Names(names).Execute()
...
func main() {
    ...
    req := apiClient.OffloadsAPI.OffloadsPatch(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `OffloadsAPI.OffloadsPatch``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `OffloadsPatch`: OffloadResponse
    fmt.Fprintf(os.Stdout, "Response from `OffloadsAPI.OffloadsPatch`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiOffloadsPatchRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **offload** | [**OffloadPatch**](OffloadPatch.md) |  | 
 **authorization** | **string** | Access token (in JWT format) required to use any API endpoint (except &#x60;/oauth2&#x60;, &#x60;/login&#x60;, and &#x60;/logout&#x60;)  | 
 **xRequestID** | **string** | Supplied by client during request or generated by server.  | 
 **contextNames** | **[]string** | Performs the operation on the context specified.  If specified, the context names must be an array of size 1, and the single element must be the name of an array in the same fleet or the name of the fleet itself. If not specified, the context will default to the array that received this request.  Other parameters provided with the request, such as names of volumes or snapshots, are resolved relative to the provided &#x60;context&#x60;.  | 
 **names** | **[]string** | Performs the operation on the unique names specified. Enter multiple names in comma-separated format. For example, &#x60;name01,name02&#x60;.  If there is not at least one resource that matches each of the elements of &#x60;names&#x60;, then an error is returned, except when creating new resources.  | 
...
> OffloadResponse OffloadsPost(ctx).Offload(offload).Authorization(authorization).XRequestID(xRequestID).ContextNames(contextNames).Initialize(initialize).Names(names).Execute()
