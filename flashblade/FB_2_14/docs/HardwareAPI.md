...
------------- | ------------- | -------------
[**HardwareGet**](HardwareAPI.md#HardwareGet) | **Get** /api/2.14/hardware | GET hardware
[**HardwarePatch**](HardwareAPI.md#HardwarePatch) | **Patch** /api/2.14/hardware | PATCH hardware



## HardwareGet

> HardwareGetResponse HardwareGet(ctx).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
...
> HardwareResponse HardwarePatch(ctx).Hardware(hardware).XRequestID(xRequestID).Ids(ids).Names(names).Execute()
