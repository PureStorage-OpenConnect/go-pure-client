...
------------- | ------------- | -------------
[**RolesGet**](RolesAPI.md#RolesGet) | **Get** /api/2.12/roles | GET roles



## RolesGet

> RoleGetResponse RolesGet(ctx).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
...
func main() {
    ...
    req := apiClient.RolesAPI.RolesGet(context.Background())
    ...
    // response from `RolesGet`: RoleGetResponse
...
[**RoleGetResponse**](RoleGetResponse.md)
