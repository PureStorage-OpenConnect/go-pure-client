...
------------- | ------------- | -------------
[**RolesGet**](RolesAPI.md#RolesGet) | **Get** /api/2.23/roles | GET roles



## RolesGet

> RoleGetResponse RolesGet(ctx).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
