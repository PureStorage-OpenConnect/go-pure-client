...
------------- | ------------- | -------------
[**AuditsGet**](AuditsAPI.md#AuditsGet) | **Get** /api/2.28/audits | GET audits



## AuditsGet

> AuditGetResponse AuditsGet(ctx).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
