...
------------- | ------------- | -------------
[**FileSystemGroupQuotasGet**](UserGroupQuotasAPI.md#FileSystemGroupQuotasGet) | **Get** /api/2.28/file-system-group-quotas | GET /file-system-group-quotas
[**FileSystemUserQuotasGet**](UserGroupQuotasAPI.md#FileSystemUserQuotasGet) | **Get** /api/2.28/file-system-user-quotas | GET /file-system-user-quotas



## FileSystemGroupQuotasGet

> FileSystemGroupQuotaGetResponse FileSystemGroupQuotasGet(ctx).XRequestID(xRequestID).AllowErrors(allowErrors).ContextNames(contextNames).ContinuationToken(continuationToken).FileSystemIds(fileSystemIds).FileSystemNames(fileSystemNames).Filter(filter).Gids(gids).GroupNames(groupNames).Limit(limit).Offset(offset).Sort(sort).Execute()
...
func main() {
    ...
    req := apiClient.UserGroupQuotasAPI.FileSystemGroupQuotasGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `UserGroupQuotasAPI.FileSystemGroupQuotasGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `FileSystemGroupQuotasGet`: FileSystemGroupQuotaGetResponse
    fmt.Fprintf(os.Stdout, "Response from `UserGroupQuotasAPI.FileSystemGroupQuotasGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiFileSystemGroupQuotasGetRequest struct via the builder pattern
...
[**FileSystemGroupQuotaGetResponse**](FileSystemGroupQuotaGetResponse.md)
...
func main() {
    ...
    req := apiClient.UserGroupQuotasAPI.FileSystemUserQuotasGet(context.Background())
    // Set request parameters ...

    resp, r, err := req.Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `UserGroupQuotasAPI.FileSystemUserQuotasGet``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `FileSystemUserQuotasGet`: FileSystemUserQuotaGetResponse
    fmt.Fprintf(os.Stdout, "Response from `UserGroupQuotasAPI.FileSystemUserQuotasGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiFileSystemUserQuotasGetRequest struct via the builder pattern
...
[**FileSystemUserQuotaGetResponse**](FileSystemUserQuotaGetResponse.md)
