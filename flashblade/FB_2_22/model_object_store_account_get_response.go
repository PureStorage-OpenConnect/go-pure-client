/*
FlashBlade REST API

OpenAPI specification for FlashBlade REST API, developed by Everpure, Inc. (https://www.everpuredata.com/).

API version: 2.22
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gopureclient

import (
	"encoding/json"
)

// checks if the ObjectStoreAccountGetResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ObjectStoreAccountGetResponse{}

// ObjectStoreAccountGetResponse struct for ObjectStoreAccountGetResponse
type ObjectStoreAccountGetResponse struct {
	// Continuation token that can be provided in the `continuation_token` query param to get the next page of data. If you use the `continuation_token` to page through data you are guaranteed to get all items exactly once regardless of how items are modified. If an item is added or deleted during the pagination then it may or may not be returned. The `continuation_token` is generated if the `limit` is less than the remaining number of items, and the default sort is used (no sort is specified).
	ContinuationToken *string `json:"continuation_token,omitempty"`
	// Total number of items after applying `filter` params.
	TotalItemCount *int32 `json:"total_item_count,omitempty"`
	// The list of errors encountered when attempting to perform an operation.
	Errors []ErrorContextResponseErrors `json:"errors,omitempty"`
	// A list of object store account objects.
	Items []ObjectStoreAccount `json:"items,omitempty"`
	// If `total_only` query param is `true`, then this field will be a total of all records after filtering and no items will be returned. If `total_only` is `false`, then it will be a total of all records on this page.
	Total *ObjectStoreAccount `json:"total,omitempty"`
}

// NewObjectStoreAccountGetResponse instantiates a new ObjectStoreAccountGetResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewObjectStoreAccountGetResponse() *ObjectStoreAccountGetResponse {
	this := ObjectStoreAccountGetResponse{}
	return &this
}

// NewObjectStoreAccountGetResponseWithDefaults instantiates a new ObjectStoreAccountGetResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewObjectStoreAccountGetResponseWithDefaults() *ObjectStoreAccountGetResponse {
	this := ObjectStoreAccountGetResponse{}
	return &this
}

// GetContinuationToken returns the ContinuationToken field value if set, zero value otherwise.
func (o *ObjectStoreAccountGetResponse) GetContinuationToken() string {
	if o == nil || IsNil(o.ContinuationToken) {
		var ret string
		return ret
	}
	return *o.ContinuationToken
}

// GetContinuationTokenOk returns a tuple with the ContinuationToken field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ObjectStoreAccountGetResponse) GetContinuationTokenOk() (*string, bool) {
	if o == nil || IsNil(o.ContinuationToken) {
		return nil, false
	}
	return o.ContinuationToken, true
}

// HasContinuationToken returns a boolean if a field has been set.
func (o *ObjectStoreAccountGetResponse) HasContinuationToken() bool {
	if o != nil && !IsNil(o.ContinuationToken) {
		return true
	}

	return false
}

// SetContinuationToken gets a reference to the given string and assigns it to the ContinuationToken field.
func (o *ObjectStoreAccountGetResponse) SetContinuationToken(v string) {
	o.ContinuationToken = &v
}

// GetTotalItemCount returns the TotalItemCount field value if set, zero value otherwise.
func (o *ObjectStoreAccountGetResponse) GetTotalItemCount() int32 {
	if o == nil || IsNil(o.TotalItemCount) {
		var ret int32
		return ret
	}
	return *o.TotalItemCount
}

// GetTotalItemCountOk returns a tuple with the TotalItemCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ObjectStoreAccountGetResponse) GetTotalItemCountOk() (*int32, bool) {
	if o == nil || IsNil(o.TotalItemCount) {
		return nil, false
	}
	return o.TotalItemCount, true
}

// HasTotalItemCount returns a boolean if a field has been set.
func (o *ObjectStoreAccountGetResponse) HasTotalItemCount() bool {
	if o != nil && !IsNil(o.TotalItemCount) {
		return true
	}

	return false
}

// SetTotalItemCount gets a reference to the given int32 and assigns it to the TotalItemCount field.
func (o *ObjectStoreAccountGetResponse) SetTotalItemCount(v int32) {
	o.TotalItemCount = &v
}

// GetErrors returns the Errors field value if set, zero value otherwise.
func (o *ObjectStoreAccountGetResponse) GetErrors() []ErrorContextResponseErrors {
	if o == nil || IsNil(o.Errors) {
		var ret []ErrorContextResponseErrors
		return ret
	}
	return o.Errors
}

// GetErrorsOk returns a tuple with the Errors field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ObjectStoreAccountGetResponse) GetErrorsOk() ([]ErrorContextResponseErrors, bool) {
	if o == nil || IsNil(o.Errors) {
		return nil, false
	}
	return o.Errors, true
}

// HasErrors returns a boolean if a field has been set.
func (o *ObjectStoreAccountGetResponse) HasErrors() bool {
	if o != nil && !IsNil(o.Errors) {
		return true
	}

	return false
}

// SetErrors gets a reference to the given []ErrorContextResponseErrors and assigns it to the Errors field.
func (o *ObjectStoreAccountGetResponse) SetErrors(v []ErrorContextResponseErrors) {
	o.Errors = v
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *ObjectStoreAccountGetResponse) GetItems() []ObjectStoreAccount {
	if o == nil || IsNil(o.Items) {
		var ret []ObjectStoreAccount
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ObjectStoreAccountGetResponse) GetItemsOk() ([]ObjectStoreAccount, bool) {
	if o == nil || IsNil(o.Items) {
		return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *ObjectStoreAccountGetResponse) HasItems() bool {
	if o != nil && !IsNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []ObjectStoreAccount and assigns it to the Items field.
func (o *ObjectStoreAccountGetResponse) SetItems(v []ObjectStoreAccount) {
	o.Items = v
}

// GetTotal returns the Total field value if set, zero value otherwise.
func (o *ObjectStoreAccountGetResponse) GetTotal() ObjectStoreAccount {
	if o == nil || IsNil(o.Total) {
		var ret ObjectStoreAccount
		return ret
	}
	return *o.Total
}

// GetTotalOk returns a tuple with the Total field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ObjectStoreAccountGetResponse) GetTotalOk() (*ObjectStoreAccount, bool) {
	if o == nil || IsNil(o.Total) {
		return nil, false
	}
	return o.Total, true
}

// HasTotal returns a boolean if a field has been set.
func (o *ObjectStoreAccountGetResponse) HasTotal() bool {
	if o != nil && !IsNil(o.Total) {
		return true
	}

	return false
}

// SetTotal gets a reference to the given ObjectStoreAccount and assigns it to the Total field.
func (o *ObjectStoreAccountGetResponse) SetTotal(v ObjectStoreAccount) {
	o.Total = &v
}

func (o ObjectStoreAccountGetResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ObjectStoreAccountGetResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.ContinuationToken) {
		toSerialize["continuation_token"] = o.ContinuationToken
	}
	if !IsNil(o.TotalItemCount) {
		toSerialize["total_item_count"] = o.TotalItemCount
	}
	if !IsNil(o.Errors) {
		toSerialize["errors"] = o.Errors
	}
	if !IsNil(o.Items) {
		toSerialize["items"] = o.Items
	}
	if !IsNil(o.Total) {
		toSerialize["total"] = o.Total
	}
	return toSerialize, nil
}

type NullableObjectStoreAccountGetResponse struct {
	value *ObjectStoreAccountGetResponse
	isSet bool
}

func (v NullableObjectStoreAccountGetResponse) Get() *ObjectStoreAccountGetResponse {
	return v.value
}

func (v *NullableObjectStoreAccountGetResponse) Set(val *ObjectStoreAccountGetResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableObjectStoreAccountGetResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableObjectStoreAccountGetResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableObjectStoreAccountGetResponse(val *ObjectStoreAccountGetResponse) *NullableObjectStoreAccountGetResponse {
	return &NullableObjectStoreAccountGetResponse{value: val, isSet: true}
}

func (v NullableObjectStoreAccountGetResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableObjectStoreAccountGetResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
