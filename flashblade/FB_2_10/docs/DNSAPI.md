...
------------- | ------------- | -------------
[**DnsGet**](DNSAPI.md#DnsGet) | **Get** /api/2.10/dns | GET dns
[**DnsPatch**](DNSAPI.md#DnsPatch) | **Patch** /api/2.10/dns | PATCH dns



## DnsGet

> DnsGetResponse DnsGet(ctx).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
...
> DnsResponse DnsPatch(ctx).Dns(dns).Ids(ids).Names(names).Execute()
