...
------------- | ------------- | -------------
[**SessionsGet**](SessionsAPI.md#SessionsGet) | **Get** /api/2.10/sessions | List session data



## SessionsGet

> SessionGetResponse SessionsGet(ctx).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
