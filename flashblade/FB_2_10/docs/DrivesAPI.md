...
------------- | ------------- | -------------
[**DrivesGet**](DrivesAPI.md#DrivesGet) | **Get** /api/2.10/drives | GET drives



## DrivesGet

> DriveGetResponse DrivesGet(ctx).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalOnly(totalOnly).Execute()
