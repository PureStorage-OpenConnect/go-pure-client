...
[**ApiLoginPost**](AuthorizationAPI.md#ApiLoginPost) | **Post** /api/login | Log in a user
...
> APIVersion ApiAPIVersionGet(ctx).XRequestID(xRequestID).Execute()
...
> LoginBannerGetResponse ApiLoginBannerGet(ctx).XRequestID(xRequestID).Execute()
...
> Login ApiLoginPost(ctx).APIToken(apiToken).XRequestID(xRequestID).Login(login).Execute()
...
> ApiLogoutPost(ctx).XRequestID(xRequestID).Execute()
