/*
FlashBlade REST API

OpenAPI specification for FlashBlade REST API, developed by Pure Storage, Inc. (https://www.purestorage.com/).

API version: 2.25
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gopureclient

import (
	"encoding/json"
)

// checks if the AuditFileSystemsPoliciesPost type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AuditFileSystemsPoliciesPost{}

// AuditFileSystemsPoliciesPost struct for AuditFileSystemsPoliciesPost
type AuditFileSystemsPoliciesPost struct {
	// A globally unique, system-generated ID. The ID cannot be modified and cannot refer to another resource.
	Id *string `json:"id,omitempty"`
	// A user-specified name. The name must be locally unique and can be changed.
	Name *string `json:"name,omitempty"`
	// Reference to the realms this resource belongs to. The value is set to empty array when the resource lives outside of a realm.
	Realms []FixedReference `json:"realms,omitempty"`
	// If `true`, the policy is enabled. If not specified, defaults to `true`.
	Enabled *bool `json:"enabled,omitempty"`
	// Whether the policy is defined on the local array.
	IsLocal *bool `json:"is_local,omitempty"`
	// Reference to the array where the policy is defined.
	Location *FixedReference `json:"location,omitempty"`
	// Type of the policy. Valid values include `alert`, `audit`, `bucket-access`, `cross-origin-resource-sharing`, `network-access`, `nfs`, `object-access`, `s3-export`, smb-client`, `smb-share`, `ssh-certificate-authority`, and `telemetry-metrics`.
	PolicyType *string `json:"policy_type,omitempty"`
	// Specifies the evaluation mode for auditing in this policy. Valid values are \"policy\", \"sacl\". If not specified, the default value is \"sacl\".
	ControlType *string `json:"control_type,omitempty"`
	// List of targets which will be utilized for audit log storage. These may either be file system targets or remote syslog server targets.
	LogTargets []Reference `json:"log_targets,omitempty"`
	// All of the rules that are part of this policy.
	Rules []AuditFileSystemsPolicyRuleInPolicy `json:"rules,omitempty"`
}

// NewAuditFileSystemsPoliciesPost instantiates a new AuditFileSystemsPoliciesPost object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAuditFileSystemsPoliciesPost() *AuditFileSystemsPoliciesPost {
	this := AuditFileSystemsPoliciesPost{}
	return &this
}

// NewAuditFileSystemsPoliciesPostWithDefaults instantiates a new AuditFileSystemsPoliciesPost object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAuditFileSystemsPoliciesPostWithDefaults() *AuditFileSystemsPoliciesPost {
	this := AuditFileSystemsPoliciesPost{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *AuditFileSystemsPoliciesPost) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *AuditFileSystemsPoliciesPost) SetName(v string) {
	o.Name = &v
}

// GetRealms returns the Realms field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetRealms() []FixedReference {
	if o == nil || IsNil(o.Realms) {
		var ret []FixedReference
		return ret
	}
	return o.Realms
}

// GetRealmsOk returns a tuple with the Realms field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetRealmsOk() ([]FixedReference, bool) {
	if o == nil || IsNil(o.Realms) {
		return nil, false
	}
	return o.Realms, true
}

// HasRealms returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasRealms() bool {
	if o != nil && !IsNil(o.Realms) {
		return true
	}

	return false
}

// SetRealms gets a reference to the given []FixedReference and assigns it to the Realms field.
func (o *AuditFileSystemsPoliciesPost) SetRealms(v []FixedReference) {
	o.Realms = v
}

// GetEnabled returns the Enabled field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetEnabled() bool {
	if o == nil || IsNil(o.Enabled) {
		var ret bool
		return ret
	}
	return *o.Enabled
}

// GetEnabledOk returns a tuple with the Enabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.Enabled) {
		return nil, false
	}
	return o.Enabled, true
}

// HasEnabled returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasEnabled() bool {
	if o != nil && !IsNil(o.Enabled) {
		return true
	}

	return false
}

// SetEnabled gets a reference to the given bool and assigns it to the Enabled field.
func (o *AuditFileSystemsPoliciesPost) SetEnabled(v bool) {
	o.Enabled = &v
}

// GetIsLocal returns the IsLocal field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetIsLocal() bool {
	if o == nil || IsNil(o.IsLocal) {
		var ret bool
		return ret
	}
	return *o.IsLocal
}

// GetIsLocalOk returns a tuple with the IsLocal field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetIsLocalOk() (*bool, bool) {
	if o == nil || IsNil(o.IsLocal) {
		return nil, false
	}
	return o.IsLocal, true
}

// HasIsLocal returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasIsLocal() bool {
	if o != nil && !IsNil(o.IsLocal) {
		return true
	}

	return false
}

// SetIsLocal gets a reference to the given bool and assigns it to the IsLocal field.
func (o *AuditFileSystemsPoliciesPost) SetIsLocal(v bool) {
	o.IsLocal = &v
}

// GetLocation returns the Location field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetLocation() FixedReference {
	if o == nil || IsNil(o.Location) {
		var ret FixedReference
		return ret
	}
	return *o.Location
}

// GetLocationOk returns a tuple with the Location field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetLocationOk() (*FixedReference, bool) {
	if o == nil || IsNil(o.Location) {
		return nil, false
	}
	return o.Location, true
}

// HasLocation returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasLocation() bool {
	if o != nil && !IsNil(o.Location) {
		return true
	}

	return false
}

// SetLocation gets a reference to the given FixedReference and assigns it to the Location field.
func (o *AuditFileSystemsPoliciesPost) SetLocation(v FixedReference) {
	o.Location = &v
}

// GetPolicyType returns the PolicyType field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetPolicyType() string {
	if o == nil || IsNil(o.PolicyType) {
		var ret string
		return ret
	}
	return *o.PolicyType
}

// GetPolicyTypeOk returns a tuple with the PolicyType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetPolicyTypeOk() (*string, bool) {
	if o == nil || IsNil(o.PolicyType) {
		return nil, false
	}
	return o.PolicyType, true
}

// HasPolicyType returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasPolicyType() bool {
	if o != nil && !IsNil(o.PolicyType) {
		return true
	}

	return false
}

// SetPolicyType gets a reference to the given string and assigns it to the PolicyType field.
func (o *AuditFileSystemsPoliciesPost) SetPolicyType(v string) {
	o.PolicyType = &v
}

// GetControlType returns the ControlType field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetControlType() string {
	if o == nil || IsNil(o.ControlType) {
		var ret string
		return ret
	}
	return *o.ControlType
}

// GetControlTypeOk returns a tuple with the ControlType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetControlTypeOk() (*string, bool) {
	if o == nil || IsNil(o.ControlType) {
		return nil, false
	}
	return o.ControlType, true
}

// HasControlType returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasControlType() bool {
	if o != nil && !IsNil(o.ControlType) {
		return true
	}

	return false
}

// SetControlType gets a reference to the given string and assigns it to the ControlType field.
func (o *AuditFileSystemsPoliciesPost) SetControlType(v string) {
	o.ControlType = &v
}

// GetLogTargets returns the LogTargets field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetLogTargets() []Reference {
	if o == nil || IsNil(o.LogTargets) {
		var ret []Reference
		return ret
	}
	return o.LogTargets
}

// GetLogTargetsOk returns a tuple with the LogTargets field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetLogTargetsOk() ([]Reference, bool) {
	if o == nil || IsNil(o.LogTargets) {
		return nil, false
	}
	return o.LogTargets, true
}

// HasLogTargets returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasLogTargets() bool {
	if o != nil && !IsNil(o.LogTargets) {
		return true
	}

	return false
}

// SetLogTargets gets a reference to the given []Reference and assigns it to the LogTargets field.
func (o *AuditFileSystemsPoliciesPost) SetLogTargets(v []Reference) {
	o.LogTargets = v
}

// GetRules returns the Rules field value if set, zero value otherwise.
func (o *AuditFileSystemsPoliciesPost) GetRules() []AuditFileSystemsPolicyRuleInPolicy {
	if o == nil || IsNil(o.Rules) {
		var ret []AuditFileSystemsPolicyRuleInPolicy
		return ret
	}
	return o.Rules
}

// GetRulesOk returns a tuple with the Rules field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuditFileSystemsPoliciesPost) GetRulesOk() ([]AuditFileSystemsPolicyRuleInPolicy, bool) {
	if o == nil || IsNil(o.Rules) {
		return nil, false
	}
	return o.Rules, true
}

// HasRules returns a boolean if a field has been set.
func (o *AuditFileSystemsPoliciesPost) HasRules() bool {
	if o != nil && !IsNil(o.Rules) {
		return true
	}

	return false
}

// SetRules gets a reference to the given []AuditFileSystemsPolicyRuleInPolicy and assigns it to the Rules field.
func (o *AuditFileSystemsPoliciesPost) SetRules(v []AuditFileSystemsPolicyRuleInPolicy) {
	o.Rules = v
}

func (o AuditFileSystemsPoliciesPost) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AuditFileSystemsPoliciesPost) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Realms) {
		toSerialize["realms"] = o.Realms
	}
	if !IsNil(o.Enabled) {
		toSerialize["enabled"] = o.Enabled
	}
	if !IsNil(o.IsLocal) {
		toSerialize["is_local"] = o.IsLocal
	}
	if !IsNil(o.Location) {
		toSerialize["location"] = o.Location
	}
	if !IsNil(o.PolicyType) {
		toSerialize["policy_type"] = o.PolicyType
	}
	if !IsNil(o.ControlType) {
		toSerialize["control_type"] = o.ControlType
	}
	if !IsNil(o.LogTargets) {
		toSerialize["log_targets"] = o.LogTargets
	}
	if !IsNil(o.Rules) {
		toSerialize["rules"] = o.Rules
	}
	return toSerialize, nil
}

type NullableAuditFileSystemsPoliciesPost struct {
	value *AuditFileSystemsPoliciesPost
	isSet bool
}

func (v NullableAuditFileSystemsPoliciesPost) Get() *AuditFileSystemsPoliciesPost {
	return v.value
}

func (v *NullableAuditFileSystemsPoliciesPost) Set(val *AuditFileSystemsPoliciesPost) {
	v.value = val
	v.isSet = true
}

func (v NullableAuditFileSystemsPoliciesPost) IsSet() bool {
	return v.isSet
}

func (v *NullableAuditFileSystemsPoliciesPost) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAuditFileSystemsPoliciesPost(val *AuditFileSystemsPoliciesPost) *NullableAuditFileSystemsPoliciesPost {
	return &NullableAuditFileSystemsPoliciesPost{value: val, isSet: true}
}

func (v NullableAuditFileSystemsPoliciesPost) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAuditFileSystemsPoliciesPost) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
