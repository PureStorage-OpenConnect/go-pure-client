...
type SmbSharePolicy struct {
	...
	// A reference to the workload that is managing this policy. Set the `id` or `name` of the workload to an empty string to remove the policy from the workload. The `name` and `configuration` may show as the string \"(unknown)\" briefly after a failover.
