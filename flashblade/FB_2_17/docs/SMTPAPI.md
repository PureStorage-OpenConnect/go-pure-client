...
------------- | ------------- | -------------
[**SmtpServersGet**](SMTPAPI.md#SmtpServersGet) | **Get** /api/2.17/smtp-servers | GET smtp-servers
[**SmtpServersPatch**](SMTPAPI.md#SmtpServersPatch) | **Patch** /api/2.17/smtp-servers | PATCH smtp-servers



## SmtpServersGet

> SmtpServerGetResponse SmtpServersGet(ctx).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
...
> SmtpServerResponse SmtpServersPatch(ctx).Smtp(smtp).XRequestID(xRequestID).Execute()
