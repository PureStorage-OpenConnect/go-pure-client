...
------------- | ------------- | -------------
[**AuditsGet**](AuditsAPI.md#AuditsGet) | **Get** /api/2.13/audits | GET audits



## AuditsGet

> AuditGetResponse AuditsGet(ctx).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
