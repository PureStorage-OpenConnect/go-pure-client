...
------------- | ------------- | -------------
[**BladesGet**](BladesAPI.md#BladesGet) | **Get** /api/2.13/blades | GET blades



## BladesGet

> BladeGetResponse BladesGet(ctx).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalOnly(totalOnly).Execute()
