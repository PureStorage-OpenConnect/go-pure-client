...
------------- | ------------- | -------------
[**HardwareGet**](HardwareAPI.md#HardwareGet) | **Get** /api/2.18/hardware | GET hardware
[**HardwarePatch**](HardwareAPI.md#HardwarePatch) | **Patch** /api/2.18/hardware | PATCH hardware



## HardwareGet

> HardwareGetResponse HardwareGet(ctx).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
