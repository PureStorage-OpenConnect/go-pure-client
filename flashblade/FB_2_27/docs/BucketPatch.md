...
**QosPolicy** | Pointer to [**ReferenceWritable**](ReferenceWritable.md) | The QoS policy for the bucket defines the performance controls that can be applied to the aggregate performance of all the clients accessing the bucket. If no policy is set here, no explicit performance controls are applied to the bucket beyond the system&#39;s default fairness.  | [optional] 
