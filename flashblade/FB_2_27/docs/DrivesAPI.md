...
------------- | ------------- | -------------
[**DrivesGet**](DrivesAPI.md#DrivesGet) | **Get** /api/2.27/drives | GET drives



## DrivesGet

> DriveGetResponse DrivesGet(ctx).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).TotalOnly(totalOnly).Execute()
