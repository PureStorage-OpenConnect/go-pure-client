...
------------- | ------------- | -------------
[**PoliciesAllGet**](PoliciesAllAPI.md#PoliciesAllGet) | **Get** /api/2.15/policies-all | GET policies-all
[**PoliciesAllMembersGet**](PoliciesAllAPI.md#PoliciesAllMembersGet) | **Get** /api/2.15/policies-all/members | GET policies-all/members



## PoliciesAllGet

> PolicyBaseGetResponse PoliciesAllGet(ctx).XRequestID(xRequestID).ContinuationToken(continuationToken).Filter(filter).Ids(ids).Limit(limit).Names(names).Offset(offset).Sort(sort).Execute()
