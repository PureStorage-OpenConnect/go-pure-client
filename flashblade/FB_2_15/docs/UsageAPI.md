...
------------- | ------------- | -------------
[**UsageGroupsGet**](UsageAPI.md#UsageGroupsGet) | **Get** /api/2.15/usage/groups | GET usage/groups
[**UsageUsersGet**](UsageAPI.md#UsageUsersGet) | **Get** /api/2.15/usage/users | GET usage/users



## UsageGroupsGet

> GroupQuotaGetResponse UsageGroupsGet(ctx).XRequestID(xRequestID).ContinuationToken(continuationToken).FileSystemIds(fileSystemIds).FileSystemNames(fileSystemNames).Filter(filter).Gids(gids).GroupNames(groupNames).Limit(limit).Offset(offset).Sort(sort).Execute()
...
> UserQuotaGetResponse UsageUsersGet(ctx).XRequestID(xRequestID).ContinuationToken(continuationToken).FileSystemIds(fileSystemIds).FileSystemNames(fileSystemNames).Filter(filter).Limit(limit).Offset(offset).Sort(sort).Uids(uids).UserNames(userNames).Execute()
