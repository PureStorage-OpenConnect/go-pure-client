/*
FlashBlade REST API

OpenAPI specification for FlashBlade REST API, developed by Everpure, Inc. (https://www.everpuredata.com/).

API version: 2.1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gopureclient

import (
	"encoding/json"
)

// checks if the ConnectionRelationshipPerformanceReplication type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConnectionRelationshipPerformanceReplication{}

// ConnectionRelationshipPerformanceReplication struct for ConnectionRelationshipPerformanceReplication
type ConnectionRelationshipPerformanceReplication struct {
	// A non-modifiable, globally unique ID chosen by the system.
	Id *string `json:"id,omitempty"`
	// Total bytes transmitted or received per second for all types of replication.
	Aggregate *ReplicationPerformance `json:"aggregate,omitempty"`
	// Total bytes transmitted or received per second for continuous replication. Continuous replication includes object replication.
	Continuous *ReplicationPerformance `json:"continuous,omitempty"`
	// Total bytes transmitted or received per second for periodic replication. Periodic replication includes file system replication, which is snapshot based.
	Periodic *ReplicationPerformance `json:"periodic,omitempty"`
	// Reference to a remote array.
	Remote *FixedReferenceNoResourceType `json:"remote,omitempty"`
	// Sample time in milliseconds since UNIX epoch.
	Time *int64 `json:"time,omitempty"`
}

// NewConnectionRelationshipPerformanceReplication instantiates a new ConnectionRelationshipPerformanceReplication object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConnectionRelationshipPerformanceReplication() *ConnectionRelationshipPerformanceReplication {
	this := ConnectionRelationshipPerformanceReplication{}
	return &this
}

// NewConnectionRelationshipPerformanceReplicationWithDefaults instantiates a new ConnectionRelationshipPerformanceReplication object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConnectionRelationshipPerformanceReplicationWithDefaults() *ConnectionRelationshipPerformanceReplication {
	this := ConnectionRelationshipPerformanceReplication{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *ConnectionRelationshipPerformanceReplication) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConnectionRelationshipPerformanceReplication) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *ConnectionRelationshipPerformanceReplication) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *ConnectionRelationshipPerformanceReplication) SetId(v string) {
	o.Id = &v
}

// GetAggregate returns the Aggregate field value if set, zero value otherwise.
func (o *ConnectionRelationshipPerformanceReplication) GetAggregate() ReplicationPerformance {
	if o == nil || IsNil(o.Aggregate) {
		var ret ReplicationPerformance
		return ret
	}
	return *o.Aggregate
}

// GetAggregateOk returns a tuple with the Aggregate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConnectionRelationshipPerformanceReplication) GetAggregateOk() (*ReplicationPerformance, bool) {
	if o == nil || IsNil(o.Aggregate) {
		return nil, false
	}
	return o.Aggregate, true
}

// HasAggregate returns a boolean if a field has been set.
func (o *ConnectionRelationshipPerformanceReplication) HasAggregate() bool {
	if o != nil && !IsNil(o.Aggregate) {
		return true
	}

	return false
}

// SetAggregate gets a reference to the given ReplicationPerformance and assigns it to the Aggregate field.
func (o *ConnectionRelationshipPerformanceReplication) SetAggregate(v ReplicationPerformance) {
	o.Aggregate = &v
}

// GetContinuous returns the Continuous field value if set, zero value otherwise.
func (o *ConnectionRelationshipPerformanceReplication) GetContinuous() ReplicationPerformance {
	if o == nil || IsNil(o.Continuous) {
		var ret ReplicationPerformance
		return ret
	}
	return *o.Continuous
}

// GetContinuousOk returns a tuple with the Continuous field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConnectionRelationshipPerformanceReplication) GetContinuousOk() (*ReplicationPerformance, bool) {
	if o == nil || IsNil(o.Continuous) {
		return nil, false
	}
	return o.Continuous, true
}

// HasContinuous returns a boolean if a field has been set.
func (o *ConnectionRelationshipPerformanceReplication) HasContinuous() bool {
	if o != nil && !IsNil(o.Continuous) {
		return true
	}

	return false
}

// SetContinuous gets a reference to the given ReplicationPerformance and assigns it to the Continuous field.
func (o *ConnectionRelationshipPerformanceReplication) SetContinuous(v ReplicationPerformance) {
	o.Continuous = &v
}

// GetPeriodic returns the Periodic field value if set, zero value otherwise.
func (o *ConnectionRelationshipPerformanceReplication) GetPeriodic() ReplicationPerformance {
	if o == nil || IsNil(o.Periodic) {
		var ret ReplicationPerformance
		return ret
	}
	return *o.Periodic
}

// GetPeriodicOk returns a tuple with the Periodic field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConnectionRelationshipPerformanceReplication) GetPeriodicOk() (*ReplicationPerformance, bool) {
	if o == nil || IsNil(o.Periodic) {
		return nil, false
	}
	return o.Periodic, true
}

// HasPeriodic returns a boolean if a field has been set.
func (o *ConnectionRelationshipPerformanceReplication) HasPeriodic() bool {
	if o != nil && !IsNil(o.Periodic) {
		return true
	}

	return false
}

// SetPeriodic gets a reference to the given ReplicationPerformance and assigns it to the Periodic field.
func (o *ConnectionRelationshipPerformanceReplication) SetPeriodic(v ReplicationPerformance) {
	o.Periodic = &v
}

// GetRemote returns the Remote field value if set, zero value otherwise.
func (o *ConnectionRelationshipPerformanceReplication) GetRemote() FixedReferenceNoResourceType {
	if o == nil || IsNil(o.Remote) {
		var ret FixedReferenceNoResourceType
		return ret
	}
	return *o.Remote
}

// GetRemoteOk returns a tuple with the Remote field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConnectionRelationshipPerformanceReplication) GetRemoteOk() (*FixedReferenceNoResourceType, bool) {
	if o == nil || IsNil(o.Remote) {
		return nil, false
	}
	return o.Remote, true
}

// HasRemote returns a boolean if a field has been set.
func (o *ConnectionRelationshipPerformanceReplication) HasRemote() bool {
	if o != nil && !IsNil(o.Remote) {
		return true
	}

	return false
}

// SetRemote gets a reference to the given FixedReferenceNoResourceType and assigns it to the Remote field.
func (o *ConnectionRelationshipPerformanceReplication) SetRemote(v FixedReferenceNoResourceType) {
	o.Remote = &v
}

// GetTime returns the Time field value if set, zero value otherwise.
func (o *ConnectionRelationshipPerformanceReplication) GetTime() int64 {
	if o == nil || IsNil(o.Time) {
		var ret int64
		return ret
	}
	return *o.Time
}

// GetTimeOk returns a tuple with the Time field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ConnectionRelationshipPerformanceReplication) GetTimeOk() (*int64, bool) {
	if o == nil || IsNil(o.Time) {
		return nil, false
	}
	return o.Time, true
}

// HasTime returns a boolean if a field has been set.
func (o *ConnectionRelationshipPerformanceReplication) HasTime() bool {
	if o != nil && !IsNil(o.Time) {
		return true
	}

	return false
}

// SetTime gets a reference to the given int64 and assigns it to the Time field.
func (o *ConnectionRelationshipPerformanceReplication) SetTime(v int64) {
	o.Time = &v
}

func (o ConnectionRelationshipPerformanceReplication) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConnectionRelationshipPerformanceReplication) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Aggregate) {
		toSerialize["aggregate"] = o.Aggregate
	}
	if !IsNil(o.Continuous) {
		toSerialize["continuous"] = o.Continuous
	}
	if !IsNil(o.Periodic) {
		toSerialize["periodic"] = o.Periodic
	}
	if !IsNil(o.Remote) {
		toSerialize["remote"] = o.Remote
	}
	if !IsNil(o.Time) {
		toSerialize["time"] = o.Time
	}
	return toSerialize, nil
}

type NullableConnectionRelationshipPerformanceReplication struct {
	value *ConnectionRelationshipPerformanceReplication
	isSet bool
}

func (v NullableConnectionRelationshipPerformanceReplication) Get() *ConnectionRelationshipPerformanceReplication {
	return v.value
}

func (v *NullableConnectionRelationshipPerformanceReplication) Set(val *ConnectionRelationshipPerformanceReplication) {
	v.value = val
	v.isSet = true
}

func (v NullableConnectionRelationshipPerformanceReplication) IsSet() bool {
	return v.isSet
}

func (v *NullableConnectionRelationshipPerformanceReplication) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConnectionRelationshipPerformanceReplication(val *ConnectionRelationshipPerformanceReplication) *NullableConnectionRelationshipPerformanceReplication {
	return &NullableConnectionRelationshipPerformanceReplication{value: val, isSet: true}
}

func (v NullableConnectionRelationshipPerformanceReplication) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConnectionRelationshipPerformanceReplication) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
